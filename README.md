...
> 💡 LLM은 포함되지 않는다. docloom은 **"미리보기 + 왕복 채널"만** 책임지고,
> HTML을 편집하는 일은 호출하는 쪽이 한다. → **LLM이 읽고 고칠 수 있는 HTML로 바꿨다가
> 원본 양식 그대로 되돌리는** 용도에 맞다.
...
> ⚠️ **두 HTML을 구분하세요.**
> `previewHtml()`은 보기 전용(왕복 ✗), `encode().html`은 편집/왕복용 본문 조각(`manifest`와 짝).
...
| `validateHtml(html, palette)` | LLM 편집 HTML 안전 교정 |
